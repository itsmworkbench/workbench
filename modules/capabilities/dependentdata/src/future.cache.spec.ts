import { CacheExecuteAsync, CallStatus, defaultCacheCleanup, defaultCacheExecuteAsync, defaultCacheFound, defaultCacheSearch, FutureCache, futureCache, getOrUpdateFromFutureCache } from "./future.cache";
import { callListeners } from "@itsmworkbench/utils";
import { defaultRetryPolicy } from "./retry";

let cs1: CallStatus<string> = { context: 'context1', params: [ 1, 2, 3 ], value: Promise.resolve ( 'value1' ), time: 1000, valueStatus: 'fulfilled' };
let cs2: CallStatus<string> = { context: 'context2', params: [ 4, 5, 6 ], value: Promise.resolve ( 'value2' ), time: 2000, valueStatus: 'fulfilled' };
...
describe ( 'defaultCacheCleanup', () => {
  ...
  it ( 'should retain all entries if none are past the TTL', () => {
    const mockTimeService = () => 10000;
    const ttl = 5000;
    const mockCache: FutureCache<string> = {
      ...emptyCache,
      timeService: mockTimeService,
      ttl: ttl,
      cache: [
  ...
  it ( 'should retain all entries if TTL is null', () => {
    // Current time is 10000, TTL is null, meaning no entries should be removed regardless of their time
    const mockTimeService = () => 10000;
    const mockCache: FutureCache<string> = {
      ...emptyCache,
      timeService: mockTimeService,
      ttl: null, // TTL is not set
      cache: [
        { context: 'context1', params: [ 1 ], value: Promise.resolve ( 'value1' ), time: 5000 }, // Older but should remain
        { context: 'context2', params: [ 2 ], value: Promise.resolve ( 'value2' ), time: 9500 }, // Recent and should remain
      ]
    };
    ...
    expect ( result ).toEqual ( expect.arrayContaining ( [
      expect.objectContaining ( { context: 'context1', time: 5000 } ),
      expect.objectContaining ( { context: 'context2', time: 9500 } )
    ] ) );
    expect ( result.length ).toBe ( 2 ); // Expect all entries to remain
  } );
} );
...
describe ( 'defaultCacheExecuteAsync', () => {
  it ( 'should call raw and handle result correctly', async () => {
    ...
    const cache = { ...emptyCache, listeners: [] };
    const context = 'testContext';
    const params = [ 1, 2, 3 ];
    const callStatus: CallStatus<string> = { context, params, time: 0, valueStatus: undefined, value: undefined }

    const executeAsync = defaultCacheExecuteAsync ();
    const result = await executeAsync ( cache, callStatus, raw );
  ...
  it ( 'should call raw and handle error correctly', async () => {
    ...
    await expect ( executeAsync ( cache, callStatus, raw ) ).rejects.toThrow ( 'Failed' );
...
describe ( 'getOrUpdateFromFutureCache', () => {
  ...
  beforeEach ( () => {
    cache = {
      ...
      execute: async ( cache, callStatus, raw ) => raw ( callStatus.params )
    };
  } );
