...
export type PathToLensFn<Main, Child> = (path: LensPath | string) => LensAndPath<Main, Child>;

export function pathToLens<Main, Child>(path: LensPath | string): LensAndPath<Main, Child> {
    ...
    const buildLens = (lens: LensAndPath<any, any>, pathPart: LensPathPart): LensAndPath<any, any> => {
        if (typeof pathPart === 'string') {
            ...
            for (const [key, subPath] of Object.entries(pathPart as ComposedPathPart)) {
                childLenses[key] = pathToLens(subPath);
            }
