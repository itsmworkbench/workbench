...
export function throwErrorIfNotBase64(value: string): void {
    if (!value) throw new Error("String is empty; expected a Base64-encoded value.");
    if (!base64Regex.test(value)) throw new Error(`Invalid Base64 string: "${value}"`);
}
