...
import {BasicAuthentication, basicPasswordL, basicUsernameL} from "./basicAuthentication";
...
export const bearerAuthenticationPlugin: AuthenticationPlugin<BearerAuthentication> = {
    ...
    objectDefn: {
        layout: [1,1],
        fields: {
            method: {lens: lensBuilder<BearerAuthentication>().focusOn('method'), editable: false},
            apiKey: {lens: lensBuilder<BearerAuthentication>().focusOn('credentials').focusOn('apiKey'), fieldType: 'encrypted'},
        }
    }
};